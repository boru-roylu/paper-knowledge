...
- [FireRedASR2S](../papers/arxiv_2603_10420/)：不是 audio judge paper，但可作 generated speech evaluation component：ASR transcript fidelity、LID language/dialect adherence、punctuation/segmentation quality、mVAD speech/singing/music contamination checks。使用時要把 ASR evaluator bias 與 generator failure 分開。
...
- [DataComp-VLM](../papers/arxiv_2606_28551/)：VLM-side data/eval benchmark methodology。可借它的 Validation/Core/Extended eval tiers、stability/monotonicity benchmark filtering、train-test decontamination 和 scale-aware data mixture sweeps，設計 audio generation / TTS / full-duplex evaluator benchmark。
...
- [Mechanistic Data Attribution](../papers/arxiv_2601_21996/)：不是 audio paper，但提供 training-time mechanism attribution。FlashTrace 解釋 inference-time decision 依賴哪些 input spans；MDA 則問某個 internal unit / behavior 是由哪些 training samples 催化形成。未來可借來 audit audio judge / speech LLM：某個 rubric behavior、grounded span attribution、overlap detection 或 reward-hacking shortcut，到底來自哪些 training examples。
