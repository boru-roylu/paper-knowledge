...
- [NaturalSpeech 2](../papers/arxiv_2304_09116/)：multi-step latent diffusion TTS teacher / baseline。它用 continuous RVQ-derived latent vectors + 150-step ODE sampling 做 zero-shot TTS，作者 future work 也指向 consistency models；適合拿來思考 one-step / few-step TTS distillation 是否能保住 WER、speaker similarity、prompt prosody 和 singing/voice-conversion capability。
...
- [MeanFlow Transformers with Representation Autoencoders](../papers/arxiv_2511_13019/)：image-side MeanFlow + RAE reference。重點不是 image FID 本身，而是它把 one-step/few-step generation 的報告方式擴展成 quality、decoder GFLOPS、total sampling cost、training GPU-days、guidance complexity；CMT initialization + MFD/MFT bootstrapping 也可借到 one-step audio / TTS latent generation。
- [DiffusionBench](../papers/arxiv_2606_24888/)：holistic DiT benchmark。它把 MeanFlow one-/few-step 納入 ImageNet + T2I evaluation，結果提醒 one-step 方法不能只報單一 FID；audio 版也要同時報 quality、conditional correctness、latency、training cost、speaker/content/overlap metrics。
- [GEAR](../papers/arxiv_2606_32039/)：image-side discrete AR / tokenizer reference。它不是 one-step paper，但對 one-step audio 很重要：one-step / low-latency generator 對 token/latent learnability 更敏感，GEAR 提醒我們應先挑或 tune 一個 downstream generator 容易學的 tokenizer，而不是直接用 reconstruction 最好的 codec。
- [DGPO](../papers/arxiv_2510_08425/)：不是 one-step generator，但它把 reward post-training 接到 deterministic ODE / few-step diffusion rollout，而不是昂貴的 stochastic SDE policy rollout。若 one-step / few-step audio generator 有 AnyAudio-Judge-style reward，可把 group preference optimization 當成 fast alignment route。
