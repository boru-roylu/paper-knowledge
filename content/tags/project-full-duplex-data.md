...
- [SAM Audio](../papers/arxiv_2512_18099/)：open-domain promptable source separation。對 mono dialogue -> dual-channel data 是重要 external baseline：text prompts like `woman speaking`, visual masks for target speaker, and span prompts for overlap/backchannel intervals can guide extraction; SAJ's overlap/difficulty axes can evaluate cleanup. 但它不是 dialogue-specific diarization system，仍需要 speaker tracking、ASR、turn alignment。
- [DuplexSLA](../papers/arxiv_2605_20755/)：full-duplex Speech-Language-Action model。它不是 mono-to-dual separation paper，但提供很重要的 downstream schema：user audio、assistant audio、action/event channel 全部對齊到 160 ms chunk timeline，可作為 dual-channel dialogue generator 的 event/action supervision 參考。
...
- [WenetSpeech-Yue](../papers/arxiv_2509_03959/)：不是 full-duplex separation paper，但提供 transcript/metadata pipeline reference：multi-ASR hypotheses -> normalization -> ROVER voting -> confidence -> Qwen3-4B correction -> character-level timestamps。可借來把 mono dialogue recovery 的 transcript uncertainty 保存下來，避免把 ASR/diarization pseudo-label 當 ground truth。
- [FireRedASR](../papers/arxiv_2501_14350/)：open Mandarin ASR baseline。它不處理 diarization / separation，但可作 transcript recovery / ASR disagreement baseline；FireRedASR-LLM 的 prompt+speech -> transcript setup 也適合測試 LLM-ASR 在 interruption、overlap、disfluency 場景是否 hallucinate 或過度 normalize。
- [FireRedASR2S](../papers/arxiv_2603_10420/)：更完整的 transcript infrastructure。它輸出 VAD segments、ASR confidence、token/word timestamps、language/dialect labels、punctuated text，mVAD 還標 speech/singing/music。對 full-duplex project 可作 mono dialogue preprocessing 的 metadata layer，但仍缺 diarization、overlap speech detection/separation 和 speaker-wise transcript。
