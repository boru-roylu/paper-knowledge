...
[LoSATok](../papers/arxiv_2605_27840/) 進一步把這個問題擴到 cross-domain audio：1280-d semantic feature 對 understanding 很強，但直接作為 generation target 會加重 DiT modeling burden；作者用 SemBo 壓成 128-d semantic bottleneck，再用 dual-level semantic supervision 做 semantic-acoustic tokenizer。它的關鍵 evidence 是：reconstruction 不是最強，卻在固定 208M DiT 下讓 TTS / text-to-audio / text-to-music generation 更好，且 TTA CLAP 約 21K-28K steps 就接近其他 tokenizer 150K steps 的水準。這是 **representation learnability / compute-to-quality** 的直接 audio-side case。
...
3. **Continuous speech encoder**
   ...
   - LoSATok 類 low-dimensional semantic-acoustic continuous latent
...
- [Eta-WavLM](../papers/arxiv_2505_19273/)：speaker leakage / content preservation baseline。它用 frozen WavLM + frozen ECAPA-TDNN speaker embedding 做 linear residualization，把 WavLM feature 中可由 speaker embedding 線性預測的 component 扣掉；speaker classification accuracy 從 82.30% 降到 55.73%，並在 Any-to-One VC 上改善 WER、target speaker similarity、source speaker leakage 和 MOS。對本 project 來說，它提供一個便宜、可重現的 representation probe：不要只問 reconstruction，而要問 latent 裡 source speaker identity 是否會污染 downstream generator。
- [SODA](../papers/arxiv_2602_16687/)：discrete audio foundation model scaling study。它把 Mimi semantic/acoustic tokens 和 text tokens 做 utterance-level interleaving，並顯示 validation NLL 能強烈預測 ASR/TTS/SIM；對本 project 來說，這是 representation learnability / compute-to-quality 的 direct audio-side evidence。
...
- [LoSATok](../papers/arxiv_2605_27840/)：cross-domain audio-side core evidence。它把 MiDashengLM 1280-d semantic feature 透過 SemBo 壓成 128-d，並用 dual-level semantic supervision 訓練 continuous semantic-acoustic tokenizer。最重要的點是 LoSATok reconstruction 不如最強 acoustic tokenizer，但 fixed small DiT 下 generation 更好，且 TTA CLAP 收斂速度大幅領先。這支持本 project 把 metric 從 reconstruction fidelity 擴展到 **downstream DiT learnability、model-size sensitivity、steps-to-quality**。
...
- [DiffusionBench](../papers/arxiv_2606_24888/)：image-side holistic evaluation reference。它把 RAE / VAE / pixel-space / MeanFlow 放到 ImageNet + T2I 雙軸比較，證明單一 ImageNet FID ranking 不能代表 conditional generation capability；對 speech representation benchmark 的啟發是 audio codec / VAE 也要跨 TTS、voice cloning、instruction following、mixed audio tasks 評估。
- [MeanFlow Transformers with Representation Autoencoders](../papers/arxiv_2511_13019/)：RAE latent 的 one-step / few-step extension。它顯示 representation evaluation 還應納入 decoder compute、sampling GFLOPS、training GPU-days、是否需要 guidance、以及 naive one-step training 是否 gradient explosion。這些都可轉成 speech/audio representation benchmark 的 compute-to-quality metrics。
