...
- [WenetSpeech-Yue](../papers/arxiv_2509_03959/)：Cantonese ASR/TTS corpus pipeline，但 data engineering 很值得借。WenetSpeech-Pipe 把 VAD segmentation、pyannote diarization、age/gender、DNSMOS/SNR、multi-ASR hypotheses、text normalization、ROVER confidence、Qwen3-4B correction、character timestamps 都存進 metadata；對 English TTS pipeline 的啟發是保存 full provenance，不只保存最後 transcript。
...
- [Eta-WavLM](../papers/arxiv_2505_19273/)：不是 data cleaning paper，但它提醒 TTS pipeline 的 representation cache 要明確區分 content representation 和 speaker identity。若用 WavLM / HuBERT 類 SSL feature 當 downstream target，應額外保存 speaker embedding、speaker leakage probe score、ASR WER/PER、target/source speaker similarity；這樣才能判斷某個 utterance 的 content latent 是否被 source speaker identity 污染。
